...
const Fiche = ({logements}) => {
  ...
  return (
    logementInfos && <div className="container fiche">
      <div className="location-and-tags" style={{marginBottom:16}}>
        <h1 style={{fontSize:18, textWrap: 'balance', marginBottom:5}}>{logementInfos.title}</h1>
        <h2 style={{fontSize:14, marginBottom:10}}>{logementInfos.location}</h2>
        <div className="tags" style={{display:'flex', flexWrap: 'wrap', gap:10}}>
          {
            logementInfos.tags.map((tag) => 
              <div className="tag" key={tag} style={{display:'flex', justifyContent: 'center', alignItems: 'center', backgroundColor: '#FF6060', borderRadius:5, minWidth:84, height:18, padding: '0 7px', color: '#FFFFFF', fontSize:10}}>{tag}</div>
            )
          }
        </div>
      </div>
      <div className="host-and-rate" style={{display:'flex', flexDirection: 'row-reverse', justifyContent: 'space-between', alignItems: 'center', marginBottom:13}}>
        <div className="host" style={{
            display:'flex',
            alignItems:'center',
            gap:10 
          }}>
          <div className="host-name" style={{textAlign:'right', fontSize:12}}>
            {
              splitNames.map((word, index) => <div key={index}>{word}</div>)
            }
          </div>
          <img className="host-picture" src={logementInfos.host.picture} alt="" style={{
            width:32,
            height:32,
            borderRadius:16,
            objectFit:'cover'
        ...
        <div className="rate">
          <svg width="196" height="36" viewBox="0 0 196 36" fill="none" xmlns="http://www.w3.org/2000/svg" style={{width:98, height:18}} aria-labelledby="ratingTitle">
